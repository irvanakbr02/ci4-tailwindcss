...
<div class="w-full p-10 ">
    <div class="bg-white overflow-auto">
        <h1 class="text-base mt-8 mb-3 mx-12 md:text-xl lg:text-2xl font-semibold text-slate-700"> <span class="block font-bold text-slate-700 text-3xl">Ubah Data Wisata Tambaknegara</span></h1>
        <div class="leading-loose">

            <form action="/admin/kategori/wisata/update/<?= $wisata['id']; ?>" method="POST" enctype="multipart/form-data" class="p-10 bg-white rounded shadow-xl">
                <?= csrf_field(); ?>
                <input type="hidden" name="slug" id="slug" value="<?= $wisata['slug']; ?>">
                <input type="hidden" name="fotoLama" value="<?= $wisata['foto']; ?>">
                <div class="mb-2">
                    <label class="block text-sm text-gray-600 mb-3  " for="nama">Lokasi Wisata</label>
                    <input value="<?= (old('nama')) ? old('nama') : $wisata['nama'] ?>" class="<?= ($validation->hasError('nama')) ? 'is-invalid' : '' ?> w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="nama" name="nama" type="text" required="" aria-label="Name">
                </div>
