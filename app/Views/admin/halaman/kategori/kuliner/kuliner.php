...
<div class="w-full p-10 ">
                ...
                <?php foreach ($kuliner as $kuliner) : ?>
                    <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                        <th scope="row" class="py-8 px-6 font-semibold text-gray-900 whitespace-nowrap dark:text-white">
                            <img src="/img/<?= $kuliner['foto']; ?>" class="w-96 h-44" alt="">
                        </th>
                        <th scope="row" class="py-8 px-8 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <?php echo substr($kuliner['nama'], 0, 30) . "..";  ?>
                        </th>
                        <th scope="row" class="py-8 px-8 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <?php echo substr($kuliner['deskripsi'], 0, 40) . "..";  ?>
                        </th>
                        <td class="py-8 px-8 flex items-center justify-center">
                            <a href="<?= base_url('admin/kategori/kuliner/edit/' . $kuliner['slug']); ?>" class="font-medium hover:font-semibold bg-orange-300 hover:bg-orange-500 text-slate-700 py-2 px-5 rounded-full ">Edit</a>
                            <form action="/admin/kategori/kuliner/<?= $kuliner['id']; ?>" method="POST" class="inline">
                                <?= csrf_field(); ?>
    ...
    <div class="mt-8">
        <a href="/admin/kategori/kuliner/create" class="py-3 px-8  hover:bg-slate-200 duration-300 ease-in-out shadow-md bg-slate-300 text-slate-600 rounded-xl text-base font-semibold hover:font-bold hover:text-slate-700">Tambah Kuliner Baru </a>
    </div>
</div>
